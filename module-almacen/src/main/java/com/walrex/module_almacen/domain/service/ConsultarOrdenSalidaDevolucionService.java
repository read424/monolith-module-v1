...
public class ConsultarOrdenSalidaDevolucionService implements ConsultarOrdenSalidaDevolucionUseCase {
    ...
    public Mono<PaginatedResponse<OrdenSalidaDevolucionResponse>> consultarOrdenSalidaDevolucion(ListadoOrdenSalidaDevolucionRequest request) {
    ...
    /**
     * Obtiene los datos paginados combinando la consulta de datos y el conteo total.
     */
    private Mono<PaginatedResponse<OrdenSalidaDevolucionResponse>> obtenerDatosPaginados(ListadoOrdenSalidaDevolucionRequest request) {
        return Mono.zip(
                ordenSalidaDevolucionPersistencePort.obtenerListadoOrdenSalidaDevolucion(request)
                        ...
                        .collectList(),
                ordenSalidaDevolucionPersistencePort.contarOrdenSalidaDevolucion(request)
        ).map(tuple -> {
    ...
    private Mono<Void> validarPaginacion(ListadoOrdenSalidaDevolucionRequest request) {
        return Mono.fromRunnable(() -> {
            ...
            if (!pagination.isValid()) {
                throw new IllegalArgumentException("Parámetros de paginación inválidos: página >= 0, tamaño entre 1 y 100");
            }
            ...
            if (!allowedSortDirections.contains(pagination.getSortDirection().toUpperCase())) {
                throw new IllegalArgumentException("Dirección de ordenamiento no permitida: " + pagination.getSortDirection());
            }

        ...
        public Mono<SalidaDevolucionDTO> registrarDevolucionRollos(SalidaDevolucionDTO salidaDevolucion,
                ...
                return aplicarReglasDeNegocio(salidaDevolucion)
                                ...
                                .doOnNext(resultado -> log.info(
                                                ...
                                                resultado.getArticulos().size()))
                                .doOnError(error -> log.error("❌ Error al registrar devolución: {}",
                                                error.getMessage()));
        }
                ...
                return Flux.fromIterable(salidaDevolucion.getArticulos())
                                ...
                                .flatMap(articulo -> Flux.fromIterable(articulo.getRollos())
                                                ...
                                                .filterWhen(rollo -> devolucionRollosPort
                                                                .verificarRolloYaDevuelto(
                                                                                rollo.getIdDetOrdenIngresoPeso())
                                                                .map(yaDevuelto -> {
                                                                        if (yaDevuelto) {
                                                                                // Si ya fue devuelto (true) → omitir
                                                                                // rollo
                                                                                log.debug("⚠️ Rollo {} ya fue devuelto - omitiendo del procesamiento",
                                                                                                rollo.getCodRollo());
                                                                                return false; // No incluir en el
                                                                                              // procesamiento
                                                                        } else {
                                                                                // Si NO fue devuelto (false) → incluir
                                                                                // rollo
                                                                                log.debug("✅ Rollo {} válido para devolución",
                                                                                                rollo.getCodRollo());
                                                                                return true; // Incluir en el
                                                                                             // procesamiento
                                                                        }
                                                                }))
                                                .collectList()
                                                .doOnNext(rollosValidos -> {
                                                        ...
                                                        articulo.setRollos(rollosValidos); // ✅ Actualizar la lista con
                                                                                           // solo rollos válidos
                                                        log.debug("🔄 Artículo {}: {} rollos válidos de {} originales",
                                                                        articulo.getIdArticulo(), rollosValidos.size(),
                                                                        totalOriginal);
                                                })
                ...
                return Flux.fromIterable(salidaDevolucion.getArticulos())
                                ...
                                .flatMap(articulo -> {
                                        ...
                                        var rollosSeleccionados = articulo.getRollos().stream()
                                                        .filter(rollo -> rollo.getSelected() != null
                                                                        && rollo.getSelected())
                                                        .toList();

                                        // Si no hay rollos seleccionados, omitir artículo
                                        if (rollosSeleccionados.isEmpty()) {
                                                log.debug("🔄 Artículo {} omitido - sin rollos seleccionados",
                                        ...
                                        return Flux.fromIterable(rollosSeleccionados)
                                                        .flatMap(rollo -> {
                                                                if (rollo.getPesoRollo() == null
                                                                                || rollo.getPesoRollo().compareTo(
                                                                                                java.math.BigDecimal.ZERO) <= 0) {
                                                                        return Mono.<RolloDevolucionDTO>error(

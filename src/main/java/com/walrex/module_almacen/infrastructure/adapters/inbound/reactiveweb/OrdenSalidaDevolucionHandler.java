...
/**
 * Handler reactivo para las operaciones relacionadas con órdenes de salida por
 * devolución.
 * Maneja los endpoints para consultar el listado de órdenes de salida por
 * devolución.
 */
@Component
...
public class OrdenSalidaDevolucionHandler {
    ...
    /**
     * Consulta el listado de órdenes de salida por devolución aplicando filtros
     * opcionales.
     * 
     * @param request el request con los parámetros de consulta
     * @return Mono con la respuesta del servidor
     */
    @Operation(summary = "Consultar listado paginado de órdenes de salida por devolución", description = "Obtiene el listado paginado de órdenes de salida por devolución aplicando filtros opcionales como nombre del cliente, rango de fechas, código de salida y número de guía con soporte de paginación y cache", parameters = {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Listado paginado obtenido exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = PaginatedResponse.class))),
            @ApiResponse(responseCode = "400", description = "Parámetros de consulta o paginación inválidos", content = @Content(mediaType = "application/json")),
    ...
    public Mono<ServerResponse> consultarOrdenSalidaDevolucion(
            @Parameter(description = "Request con parámetros de consulta y paginación") ServerRequest request) {
    ...
    /**
     * Extrae los filtros y parámetros de paginación del request y los convierte a
    ...
    private Mono<ListadoOrdenSalidaDevolucionRequest> extraerFiltrosConPaginacion(ServerRequest request) {
        return Mono.fromCallable(() -> {
            ListadoOrdenSalidaDevolucionRequest.ListadoOrdenSalidaDevolucionRequestBuilder builder = ListadoOrdenSalidaDevolucionRequest
                    .builder();
            ...
            request.queryParam("fechaInicio")
                    .ifPresent(fechaStr -> {
                        try {
                            ...
                            throw new IllegalArgumentException("Formato de fecha inválido para fechaInicio: " + fechaStr
            ...
            request.queryParam("fechaFin")
                    .ifPresent(fechaStr -> {
                        try {
                            ...
                            throw new IllegalArgumentException("Formato de fecha inválido para fechaFin: " + fechaStr
                                    + ". Use formato yyyy-MM-dd");
                        }
                    });
